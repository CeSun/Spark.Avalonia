...
public class Octree
{
    ...
    public Octree(float x,  float y, float z) : this(new Vector3(-x / 2, -y / 2, -z / 2), new Vector3(x / 2, y / 2, z / 2))
    {
    }
    ...
    public void InsertObject(BaseBounding Box)
    {
        if (Box.MinPoint.X < MinPoint.X || Box.MinPoint.Y < MinPoint.Y || Box.MinPoint.Z < MinPoint.Z ||
            Box.MaxPoint.X > MaxPoint.X || Box.MaxPoint.Y > MaxPoint.Y || Box.MaxPoint.Z > MaxPoint.Z)
        {
            // 根节点的情况
            if (ParentNode == null)
...
InsertCurrentNode:
    ...
    public void SphereCulling<T>(List<T> Actors, Sphere sphere)
    {
        ...
        if (_Children != null)
        {
            foreach (var child in _Children)
            {
                child.SphereCulling<T>(Actors, sphere);
            }
        }
    }
}
